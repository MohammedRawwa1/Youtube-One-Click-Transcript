...
- 📜 **Transcript** button next to Like / Share – copies the complete transcript with one click, including videos with chapters. Exceptionally long transcripts (over roughly 500k characters, such as 19-hour recordings) are copied in ordered chunks — split on chapter boundaries when the video has chapters, and on segment boundaries when it does not — so the browser clipboard can handle them reliably. If a single clipboard write is ever rejected, the copy falls back to ordered parts instead of failing.
